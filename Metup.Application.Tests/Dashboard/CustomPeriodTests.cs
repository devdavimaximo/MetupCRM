...
public class CustomPeriodTests
{
    ...
    {
        var handler = new GetDashboardOverviewQueryHandler(context.Db, context.As(context.AdminUserId, UserRole.Admin), Clock(), new ActivityFeedReader(context.Db, Clock()), new StageAnalyticsProvider(context.Db));
