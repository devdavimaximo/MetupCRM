...
/**
 * O que aconteceu na operação. Seção separada porque a ordem muda com a largura: no desktop ela
 * abre a coluna lateral; no celular ela vai para o fim, depois do que exige ação.
 */
export function RecentActivitySection({
  ...
  return (
    <section aria-labelledby="recent-heading" className={cn("flex min-h-0 flex-col px-5 pt-5 pb-2", className)}>
...
/** O que precisa ser feito. No celular é o primeiro bloco depois do cabeçalho. */
export function NextTasksSection({
  tasks,
  overdueCount,
  tasksError,
  onRetryTasks,
  ...
  return (
    <section aria-labelledby="tasks-heading" className={cn("flex min-h-0 flex-1 flex-col px-5 pt-4 pb-4", className)}>
      ...
      {overdueCount > 0 && (
        <button
          type="button"
          onClick={onSeeTasks}
          className="mb-2 flex min-h-11 w-full cursor-pointer items-center justify-between gap-2 rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-left text-sm transition-colors hover:bg-danger/15 focus-visible:focus-ring"
        >
...
/**
 * Coluna de ação do desktop: o que aconteceu na operação e o que o usuário precisa fazer a seguir,
 * num painel só. `events` é `null` quando o panorama não carregou; `tasksError` só afeta as tarefas.
 */
export function SideColumn({
  ...
  highlightedEventIds,
  ...tasks
}: RecentActivityProps & NextTasksProps) {
  return (
    <Panel className="overflow-hidden">
      <RecentActivitySection
        events={events}
        onOpenDeal={tasks.onOpenDeal}
        onSeeAllActivity={onSeeAllActivity}
        seeAllActivityRef={seeAllActivityRef}
        highlightedEventIds={highlightedEventIds}
      />
      <div className="mx-5 border-t border-line-soft" />
      <NextTasksSection {...tasks} />
    </Panel>
  )
}
